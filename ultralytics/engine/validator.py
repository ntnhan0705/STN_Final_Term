...
class BaseValidator:
    ...
    # --- Internals -----------------------------------------------------
    def _setup_from_trainer(self, trainer):
        """Use training context (model/ema/device/loaders) when called during training."""
        self.device = trainer.device
        self.data = trainer.data
        self.args.half = False  # training mode always float
    ...
    def _resolve_model(self, model):
        """Load/prepare model for standalone validation."""
    ...
    def _warmup_model(self, model, imsz):
        """Warm up model forward graph for better first-iteration timing."""
    ...
    def _compute_batch_loss(self, batch, preds):
        if not (self.training and hasattr(self.model, "loss") and callable(self.model.loss)):
            return
        if getattr(self, "skip_loss", False):
            return
        try:
    ...
    def __call__(self, trainer=None, model=None):
        ...
        if self.training:
            ...
            self._ensure_progress_bar()
            self._warmup_model(model, self._normalize_imgsz(self.args.imgsz))
