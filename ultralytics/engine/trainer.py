...
class BaseTrainer:
    ...
    def final_eval(self):
        """Evaluate final weights using the validator by reloading best/last via path."""
        from pathlib import Path
        import torch

        # 1) Chọn weight: ưu tiên best rồi tới last
        f_best = getattr(self, "best", None)
        f_last = getattr(self, "last", None)
        f = None
        if f_best and Path(f_best).exists():
            f = Path(f_best)
        elif f_last and Path(f_last).exists():
            f = Path(f_last)
        else:
            LOGGER.warning("[final_eval] No best/last weights found. Skip final eval.")
            return

        # 2) Đồng bộ vài arg cần thiết cho validator
        if hasattr(self, "args") and hasattr(self.args, "plots"):
            self.validator.args.plots = self.args.plots
        va = self.validator.args
        if not hasattr(va, "pt"):
            va.pt = True
        if not getattr(va, "batch", None):
            va.batch = getattr(self.args, "batch", 1)
        if not hasattr(va, "imgsz"):
            va.imgsz = getattr(self.args, "imgsz", 640)

        # 3) Giao PATH cho validator để tự nạp model & to(device)
        with torch.inference_mode():
            self.metrics = self.validator(model=str(f))

        # 4) Chuẩn hóa metrics + callback như cuối epoch
