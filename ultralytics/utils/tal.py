...
class TaskAlignedAssigner(nn.Module):
    ...
    def get_box_metrics(self, pd_scores, pd_bboxes, gt_labels, gt_bboxes, mask_gt):
        ...
        #overlaps[mask_gt] = self.iou_calculation(gt_boxes, pd_boxes) #Code orginal

        # ensure IOU is same dtype as overlaps to avoid fp16/float mismatch (ntnhan.0705)
        iou = self.iou_calculation(gt_boxes, pd_boxes)
        if iou.dtype != overlaps.dtype:
            iou = iou.to(overlaps.dtype)
